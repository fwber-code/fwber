...
class Wikimate {
    ...
    private function login() {
		...
		if ( $loginResult->login->result == "NeedToken" ) {
			...
			$details['lgtoken'] = $this->lgtoken = strtolower(trim($loginResult->login->token));
			$details['sessionid'] = $this->sessionid = $loginResult->login->sessionid;
			$details['cookieprefix'] = $this->cookieprefix = $loginResult->login->cookieprefix;
			$details['lgdomain'] = $this->lgdomain = ".the-nd.com";
			...
			if ( $this->debugMode ) {
				echo '<br>';
				echo '1stlogin lgtoken '.$this->lgtoken.'<br>';
				echo '1stlogin sessionid '.$this->sessionid.'<br>';
				echo '1stlogin cookieprefix '.$this->cookieprefix.'<br>';
				echo '1stlogin lgdomain '.$this->lgdomain.'<br>';
				echo '<br>';
				
				echo '<br>';

				echo '2ndlogin lgtoken '.$loginResult->login->lgtoken.'<br>';
				echo '2ndlogin sessionid '.$loginResult->login->sessionid.'<br>';
				echo '2ndlogin cookieprefix '.$loginResult->login->cookieprefix.'<br>';
				echo '2ndlogin lguserid '.$loginResult->login->lguserid.'<br>';
				echo '2ndlogin lgusername '.$loginResult->login->lgusername.'<br>';
				echo '<br>';
			}
    ...
    public function query( $array ) {
		...
		$array['action'] = 'query';
		$array['format'] = 'php';
		//$array['lgtoken'] = $this->lgtoken;
		//$array['sessionid'] = $this->sessionid;
		//$array['cookieprefix'] = $this->cookieprefix;
		//$array['lgdomain'] = $this->lgdomain;
...
class WikiPage {
    ...
    function getText($refresh=false) {
		...
		if ( $refresh ) { // we want to query the api
			...
			if ( $this->wikimate->debugMode ) {print_r($r); echo "<br><br>";}
			...
			if ( $this->wikimate->debugMode ) {print_r($page);echo "<br><br>";}
    ...
    function setText( $text, $section=null, $minor=false, $summary=null ) {
		...
		//if ( !$this->exists ) {
		//	$data['createonly'] = "false"; // createonly if not exists
		//} else {
		//	$data['nocreate'] = "false"; // don't create, it should exist
		//}
		
		if ( $this->wikimate->debugMode ) {echo "data:<br>";print_r($data);echo "<br><br>";}

		$r = $this->wikimate->edit( $data ); // the edit query
		
		$this->wikimate->newrevid = $r['edit']['newrevid'];
		
		if ( $this->wikimate->debugMode ) {echo "r:<br>";print_r($r);echo "<br><br>";}
